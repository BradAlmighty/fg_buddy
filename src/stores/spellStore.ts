...
export const useSpellStore = defineStore("spell", {
  state: (): SpellStoreState => ({
    spells: [
      ...
      },
      {
        ...
        name: "Shield",
        description:
          "The target receives +2 Armour for the rest of the game. The maximum armour rule still applies (see page 36). Multiple castings of Shield on the same target have no effect.",
        school: "TH",
        baseCast: 10,
        category: "Line of Sight",
        icon: "",
  ...
  getters: {
    ...
    getSpellsBySchool:
      (state) =>
      (school: string): Spell[] => {
        return state.spells.filter((spell) => spell.school === school);
      },
    getSpellsByCategory:
      (state) =>
      (category: string): Spell[] => {
        return state.spells.filter((spell) => spell.category === category);
      },
    getSpellById:
      (state) =>
      (id: string): Spell | undefined => {
